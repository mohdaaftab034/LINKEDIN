...
function Post({ id, author, like, comment, description, image, createdAt }) {
  ...
  return (
    <div className="w-full min-h-[200px] flex flex-col gap-[10px] bg-white rounded-lg border-[1px] border-gray-300  p-[20px] ">

      <div className='flex justify-between items-center'>

        <div
          className="flex justify-center items-start gap-2.5 cursor-pointer"
          onClick={() => handleGetProfile(author.userName)}
        >
          {/* Circular Profile Image */}
          <div className="w-[50px] h-[50px] min-w-[50px] min-h-[50px] aspect-square rounded-full overflow-hidden flex items-center justify-center">
            <img
          ...
          <div>
            ...
            <div className="text-base font-thin">{author.headline}</div>
            <div className="text-base font-normal">{moment(createdAt).fromNow()}</div>
          </div>
        </div>
            ...
            {comments.map((com) => (
              <div key={com._id} className='flex flex-col gap-[10px] border-b-2 p-[20px] border-b-gray-300' >
                <div className="w-full flex justify-start items-center gap-[10px]">
                  <div onClick={() => handleGetProfile(com.user.userName)} className='w-[40px] h-[40px] rounded-full overflow-hidden flex items-center justify-center  cursor-pointer' >
